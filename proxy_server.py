...
class Server:
    ...
    def proxy_thread(self, conn, client_addr):
        ...
        *******************************************
        ...
        request = conn.recv(config['MAX_REQUEST_LEN']).decode('utf-8')        # get the request from browser
        print("The request from web server is....... "+str(request))
        first_line = request.split('\n')[0]                   # parse the first line
        url = first_line.split(' ')[1]                        # get url
        print("The url is %s and First_line is %s........." %(url,first_line))
        ...
        try:
            ...
            s.sendall(request.encode('utf-8'))                           # send request to webserver

            while 1:
                data = s.recv(config['MAX_REQUEST_LEN'])
                #data_pos=data.('utf-8')
                #orig_data=data[:data_pos]
                print("The response data from server is..... "+str(data))          # receive data from web server
                #print("The original response data from server is..... "+str(orig_data))
                if (len(data) > 0):
